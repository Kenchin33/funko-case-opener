        ...
        {isLoggedIn && (
          <div className="crash-game-main">
                  ...
                  {inventory.map((entry, index) => {
                    ...
                    return (
                      <label key={index} className={`figure-card ${selected ? 'selected' : ''}`}>
                        <input type="checkbox" style={{ display: 'none' }} checked={selected} onChange={() => toggleSelectFigure(index)} />
            ...
            <div className="game-field">
              ...
              {isGameRunning && (
                <div className="game-animation" style={{ position: 'relative', height: '300px', overflow: 'hidden', border: '1px solid #ccc', marginTop: '20px' }}>
                  <div style={{ position: 'absolute', left: '50%', transform: 'translateX(-50%)', bottom: animationY }}>
              ...
              {gameOver && <p style={{ color: 'red', marginTop: '20px' }}>💥 Ви не встигли забрати виграш!</p>}
            </div>
          </div>
        )}
      </div>
