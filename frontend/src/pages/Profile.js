...
const Profile = () => {
  ...
  const [isSuccess, setIsSuccess] = useState(false); // для стилю повідомлення
  ...
  const showErrorMessage = (msg, success = false) => {
  ...
  const openCase = async () => {
    ...
    if (!isLoggedIn) {
      showErrorMessage('Будь ласка, увійдіть до системи, щоб відкрити кейс.', false);
      return;
    }

    if (balance < caseData.price) {
      showErrorMessage('Недостатньо коштів на балансі для відкриття кейсу.', false);
      return;
    }
    ...
    try {
      ...
      showErrorMessage(err.message, false);
      setRolling(false);
    }
  };
    ...
    <div className="case-page">
      ...
      {caseData ? (
        <>
          ...
          {resultFigure && showResult && (
            <div className="result-popup-overlay">
              <div className="result-popup">
                <button 
                  ...
                  onClick={async () => {
                    try {
                      ...
                      setShowResult(false);
                      showErrorMessage('Фігурка додана у інвентар', true);
                    } catch (err) {
                      showErrorMessage(err.message, false);
                    }
                  }}
                ...
                <h3>Ви отримали:</h3>
                <img src={resultFigure.image} alt={resultFigure.name} style={{ maxWidth: '200px' }} />
                <p style={{ color: rarityColors[resultFigure.rarity] || 'white', fontWeight: '700', fontSize: '1.1rem' }}>
                  {resultFigure.name} ({resultFigure.rarity})
                </p>

                <div className="result-buttons">
                  <button
                    className="btn btn-outline open-btn"
                    onClick={async () => {
                      ...
                      try {
                        ...
                        if (!res.ok) {
                          ...
                          throw new Error(errData.message || 'Не вдалося отримати дані користувача');
                        }
                        ...
                        setShowResult(false);
                        showErrorMessage(`Фігурку продано за ${salePrice} грн`, true);
                      } catch (err) {
                        console.error('Помилка при продажу фігурки:', err);
                        showErrorMessage('Помилка під час продажу фігурки', false);
                      }
                    }}
                  >
          ...
          <audio ref={audioRef} src="/open-case-sound.mp3" />
          <audio ref={winAudioRef} src="/win-sound.mp3" />
        </>
      ) : (
        <p>Завантаження...</p>
      )}
